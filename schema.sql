...
--
/*!50003 DROP PROCEDURE IF EXISTS `sp_suporte_init_sessao` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_suporte_init_sessao`()
BEGIN
    DECLARE chave VARCHAR(6);
    DECLARE caracteres VARCHAR(62) DEFAULT 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
    
    SET chave = CONCAT(
        SUBSTRING(caracteres, 1 + FLOOR(RAND() * 62), 1),
        SUBSTRING(caracteres, 1 + FLOOR(RAND() * 62), 1),
        SUBSTRING(caracteres, 1 + FLOOR(RAND() * 62), 1),
        SUBSTRING(caracteres, 1 + FLOOR(RAND() * 62), 1),
        SUBSTRING(caracteres, 1 + FLOOR(RAND() * 62), 1),
...
-- Dump completed on 2025-03-21 22:20:29
